...
Summary:"""
    ...
    def optimize_messages_for_tool_calls(self, messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
        """Optimize messages list to keep only the latest successful tool call during iterations."""
        if not messages:
            return messages
        ...
        for i, message in enumerate(messages):
            if message.get("role") == "assistant" and "tool_calls" in message:
                ...
                if tool_call_id and tool_call_id in tool_calls_by_id:
                    tool_calls_by_id[tool_call_id] = (tool_calls_by_id[tool_call_id][0], message)
                    # Check if this tool call was successful (no error indicators in content)
                    content = message.get("content", "")
    ...
    def optimize_tool_system_prompts(self, messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
        """Optimize messages to keep only the latest tool output schema prompts and only if the latest message is a tool response."""
        if not messages:
            return messages
        
        optimized = []
        tool_schema_prompts = []  # Track tool output schema prompts
        latest_tool_response_index = -1
        
        # First pass: find the latest tool response and collect all tool output schema prompts
        for i, message in enumerate(messages):
            if message.get("role") == "tool":
                latest_tool_response_index = i
            elif message.get("role") == "system" and message.get("content", "").startswith("Tool Output Schema for"):
                # This is a tool output schema prompt
                tool_schema_prompts.append((i, message))
        
        # Second pass: build optimized messages list
        for i, message in enumerate(messages):
            if message.get("role") == "system" and message.get("content", "").startswith("Tool Output Schema for"):
                # This is a tool output schema prompt
                # Only keep it if it's the latest one AND there's a tool response after it
                if tool_schema_prompts and i == tool_schema_prompts[-1][0] and latest_tool_response_index > i:
                    optimized.append(message)
                # Skip all other tool output schema prompts
            else:
                # Keep all non-tool output schema system messages
                optimized.append(message)
